...
int main(int argc, char **argv) // 메인 함수 (argc=갯수, argv=값, **문자열 리스트라고 생각하면된다)
{
    ...
    // 서비스 이름은 (ts_service)이고 패키지 (yh_tutorial_7) 의 (yh_srv_7) 서비스 파일을 사용한다.
    ros::ServiceClient my_service_client = nh.serviceClient<yh_tutorial_7::yh_srv_7>("ts_service");
    ...
    // 서비스를 요청하고, 응답이 정상적으로 왔을 경우, 값은 표시한다.
