...
#include "yh_tutorial_3/yh_msg_3.h" // my_msg 메세지 헤더파일
                                   // 빌드 후 자동 실행
...
{   // init = initialize(?) 시작한다는 의미.
    ...
    ros::Publisher my_pub = nh.advertise<yh_tutorial_3::yh_msg_3>("yh_topic_3", 100);  // 퍼블리셔가 큐 형태로 subscriber로 밀어넣음. 그 사이즈가 100임.
    // 위 ros::Publisher=자료형    my_pub=변수명        nh.advertise 함수 사용.
    ros::Rate loop_rate(5); // 루프 주기를 5Hz로 설정한다.
