...
#include "yh_tutorial_2/yh_msg_2.h" // my_msg 메세지 헤더파일
                                   // 빌드 후 자동 실행
...
{   // init = initialize(?) 시작한다는 의미.
    ...
    ros::Publisher my_pub = nh.advertise<yh_tutorial_2::yh_msg_2>("yh_topic_2", 100);  // 퍼블리셔가 큐 형태로 subscriber로 밀어넣음. 그 사이즈가 100임.
    // 위 ros::Publisher=자료형    my_pub=변수명
    ros::Rate loop_rate(1); // 루프 주기를 5Hz로 설정한다.
